...
def fkl_torch(angles, parent, offset, rotInd, expmapInd, device="cuda:1"):
    ...
    j_n = offset.shape[0]
    p3d = torch.from_numpy(offset).float().cuda(device).unsqueeze(0).repeat(n, 1, 1)
...
def main():
    ...
    exp1 = Variable(torch.from_numpy(np.vstack((expmap_pred, expmap_gt))).float()).float().cuda()
